...
function setup() {
  ...
  const filters = ['all', 'meta', 'google', 'site', 'ai', 'seo'].map(workFilter => new Element({ workFilter }, workFilter));
  const cards = ['meta', 'site', 'meta', 'google', 'ai', 'seo', 'site'].map(workCategory => new Element({ workCategory }));
  ...
  showcase.querySelector = key => ({ '[data-feature-prev]':prev, '[data-feature-next]':next, '[data-feature-status]':status })[key];
...
test('each filter returns matching cases and All restores the full set', () => {
  const x = setup();
  for (const filter of x.filters.slice(1)) { filter.fire('click'); assert.ok(x.cards.some(c => !c.hidden)); for (const card of x.cards) assert.equal(card.hidden, card.dataset.workCategory !== filter.dataset.workFilter); }
