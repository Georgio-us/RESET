...
// Shared contact forms and modal behavior are managed by shared-ui.js.
...
const casePageNavigation = {
  'shepit-house.html': { position: '01 / 07', title: 'SHEPIT HOUSE', previous: 'telegram-ai-crm.html', next: 'an-factor.html', nextLabel: 'АГЕНТСТВО НЕДВИЖИМОСТИ / META ADS / УКРАИНА' },
  'an-factor.html': { position: '02 / 07', title: 'AN FACTOR', previous: 'shepit-house.html', next: 'estyle-spain.html', nextLabel: 'АГЕНТСТВО НЕДВИЖИМОСТИ / SEO / ИСПАНИЯ' },
  'estyle-spain.html': { position: '03 / 07', title: 'ESTYLE SPAIN', previous: 'an-factor.html', next: 'nivellux.html', nextLabel: 'РЕМОНТНО-СТРОИТЕЛЬНАЯ КОМПАНИЯ / DIGITAL / ИСПАНИЯ' },
  'nivellux.html': { position: '04 / 07', title: 'NIVELLUX', previous: 'estyle-spain.html', next: 'dominanta-spain.html', nextLabel: 'МЕЖДУНАРОДНОЕ АГЕНТСТВО / MARKET ENTRY / ИСПАНИЯ' },
  'dominanta-spain.html': { position: '05 / 07', title: 'DOMINANTA SPAIN', previous: 'nivellux.html', next: 'ai-sales-assistant.html', nextLabel: 'ESTYLESPAIN / AI ПРОДУКТ / НЕДВИЖИМОСТЬ' },
  'ai-sales-assistant.html': { position: '06 / 07', title: 'AI SALES ASSISTANT', previous: 'dominanta-spain.html', next: 'telegram-ai-crm.html', nextLabel: 'DELMAR · HELPING · DOMSTAR / SAAS / НЕДВИЖИМОСТЬ' },
  'telegram-ai-crm.html': { position: '07 / 07', title: 'TELEGRAM AI CRM', previous: 'ai-sales-assistant.html', next: 'shepit-house.html', nextLabel: 'ДЕВЕЛОПЕР КОТТЕДЖНОГО ГОРОДКА / REAL ESTATE / УКРАИНА' },
};

const currentCaseFile = window.location.pathname.split('/').pop();
const currentCaseNavigation = casePageNavigation[currentCaseFile];

if (document.body.classList.contains('case-page') && currentCaseNavigation && !document.querySelector('.case-page-header, .reset-header')) {
  const chromeCopy = {
    ru: { home: 'Главная', system: 'Система', solutions: 'Решения', cases: 'Кейсы', discuss: 'Обсудить задачу', menu: 'Основная навигация' },
    uk: { home: 'Головна', system: 'Система', solutions: 'Рішення', cases: 'Кейси', discuss: 'Обговорити задачу', menu: 'Основна навігація' },
    en: { home: 'Home', system: 'System', solutions: 'Solutions', cases: 'Cases', discuss: 'Discuss a project', menu: 'Main navigation' },
    es: { home: 'Inicio', system: 'Sistema', solutions: 'Soluciones', cases: 'Casos', discuss: 'Hablar del proyecto', menu: 'Navegación principal' },
  }[routeLocale];
  const languageLinks = [['ru','RU'],['uk','UA'],['en','EN'],['es','ES']].map(([locale,label]) => `<a href="/${locale}/cases/${currentCaseFile}" lang="${locale}"${locale === routeLocale ? ' class="is-active" aria-current="page"' : ''}>${label}</a>`).join('');
  document.body.insertAdjacentHTML('afterbegin', `<header class="site-header case-page-header container"><a class="brand" href="/${routeLocale}/" aria-label="RESET — ${chromeCopy.home}"><span class="brand-name"><span class="brand-mark-light">RE</span><span class="brand-mark-accent">SET</span></span></a><nav class="navigation" aria-label="${chromeCopy.menu}"><a href="/${routeLocale}/#system">${chromeCopy.system}</a><a href="/${routeLocale}/#system">${chromeCopy.solutions}</a><a href="/${routeLocale}/#case-index">${chromeCopy.cases}</a></nav><div class="header-actions"><div class="language-switcher" aria-label="Language">${languageLinks}</div><a class="menu-link" href="/${routeLocale}/#contact">${chromeCopy.discuss} <span aria-hidden="true">↗</span></a></div></header><nav class="case-breadcrumbs" aria-label="Breadcrumbs"><div class="container"><a href="/${routeLocale}/">${chromeCopy.home}</a><span>/</span><a href="/${routeLocale}/#case-index">${chromeCopy.cases}</a><span>/</span><b>${currentCaseNavigation.title}</b></div></nav>`);
