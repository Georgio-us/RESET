...
const versionAssets = html => html.replace(/(<(?:link|script)\b[^>]*(?:href|src)=")([^"?]+\.(?:css|js))(?:\?[^\"]*)?("[^>]*>)/g, (match, before, url, after) => /^(https?:)?\/\//.test(url) ? match : `${before}${url}?v=20260922-ui4${after}`);

export function contactForm(locale, source) {
  const c = uiCopy[locale];
  return `<form class="reset-form" data-reset-form>
    <input type="hidden" name="source" value="${source}">
    <label>${c.name} <small>${c.optional}</small><input name="name" autocomplete="name" maxlength="120"></label>
    <label>${c.company} <small>${c.optional}</small><input name="company" autocomplete="organization" maxlength="180"></label>
    ...
    <label>${c.contact} <span aria-hidden="true">*</span><input name="contact" type="tel" autocomplete="tel" inputmode="tel" placeholder="+34 600 000 000" required maxlength="180"></label>
    <label class="reset-form-wide">${c.task} <small>${c.optional}</small><textarea name="message" rows="3" maxlength="2000"></textarea></label>
...
export function renderSiteUI(html, { locale = 'ru', path = '/', exists = () => true } = {}) {
  ...
  if (/presentation/.test(path)) return versionAssets(html.replace('</head>', '<link rel="stylesheet" href="/design-tokens.css?v=20260922-ui4"><script src="/arrow-icons.js?v=20260922-ui4" defer></script></head>'));
  ...
  html = html.replace('</head>', '<link rel="stylesheet" href="/design-tokens.css?v=20260922-ui4"><link rel="stylesheet" href="/shared-ui.css?v=20260922-ui4"><script type="module" src="/shared-ui.js?v=20260922-ui4"></script><script src="/arrow-icons.js?v=20260922-ui4" defer></script></head>');
